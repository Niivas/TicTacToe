...
public class Order1WinningStrategy implements WinningStrategy{
    ...
    private Map<Integer,Map<Character,Integer>> rowMap = new HashMap<Integer,Map<Character,Integer>>();
    private Map<Integer,Map<Character,Integer>> colMap = new HashMap<Integer,Map<Character,Integer>>();
    ...
    public boolean updateAndCheckIfWon(Character symbol, int row, int col, int size) {
        ...
        return diagMap1.getOrDefault(symbol,0) == size || diagMap2.getOrDefault(symbol,0) == size || rowMap.get(row).getOrDefault(symbol,0) == size || colMap.get(col).getOrDefault(symbol,0) == size;
    }
